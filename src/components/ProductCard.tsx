...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <article className="group flex min-w-0 flex-col border-b border-border bg-card transition-shadow duration-300 hover:shadow-[0_16px_35px_-24px_rgba(58,35,13,.65)]">
      <Link to={detailPath} className="relative block aspect-[4/3] overflow-hidden bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
      ...
      <div className="flex flex-1 flex-col px-1 py-4">
        ...
        <Link to={detailPath} className="mt-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
        ...
        <div className="mt-auto flex items-end justify-between gap-3 pt-4">
          ...
          <Link to={detailPath} className="flex min-h-11 items-center gap-1 text-sm font-bold text-foreground underline decoration-border underline-offset-4 hover:text-accent focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Details <ArrowUpRight className="h-4 w-4" /></Link>
        </div>
