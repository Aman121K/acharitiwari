...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <article className="group flex min-w-0 flex-col border-t border-[#c9b896] bg-transparent pt-3 transition-transform duration-300 hover:-translate-y-1">
      <Link to={detailPath} onClick={trackSelection} className="relative block aspect-[4/5] overflow-hidden bg-[#eee4d3] p-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
      ...
      <div className="flex flex-1 flex-col py-4">
        ...
        <Link to={detailPath} onClick={trackSelection} className="mt-2 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
          <h2 className="line-clamp-2 font-display text-2xl leading-snug tracking-[-0.02em] text-foreground transition-colors group-hover:text-primary">{product.name}</h2>
        </Link>
