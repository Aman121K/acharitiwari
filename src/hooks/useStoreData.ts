...
const mapProduct=(raw:any):Product=>{const variant=raw.variants?.find((item:any)=>item.isActive!==false)||raw.variants?.[0]||{};return{id:String(raw._id),name:raw.title||raw.name,price:Number(variant.price||0),image:raw.images?.[0]||'/placeholder.svg',description:raw.shortDescription||raw.description,category:raw.category?.name||raw.productType||'Aachar',weight:variant.size||`${raw.shipping?.weightKg||''} kg`,ingredients:raw.ingredients||[],shelfLife:raw.shipping?.shelfLife||'See label',inStock:Number(variant.inventory||0)>0}};
const mapPost=(raw:any):BlogPost=>({id:String(raw.slug||raw._id),title:raw.title,excerpt:raw.excerpt,content:raw.content,author:raw.author,date:raw.publishedAt||raw.createdAt,readTime:`${Math.max(3,Math.ceil(String(raw.content||'').split(/\s+/).length/200))} min read`,category:raw.category,image:raw.coverImage||'/brand/achari-tiwari-logo.png',likes:0,tags:raw.tags||[]});
